...
if (isset($_GET['cetak']) ) {
    ...
    $logoWidth = 32; // Lebar logo
    $margin = 10; // Margin dari tepi kanan
    $x = $pdf->GetPageWidth() - $logoWidth - $margin; // Hitung posisi X untuk logo
    $y = 10; // Posisi Y tetap 10 (dari atas)
    //$pdf->Image('C:/xampp/htdocs/perpusada/assets/images/logos/elib.png', $x, $y, $logoWidth);
    // setting jenis font yang akan digunakan
    $pdf->SetFont('Arial', 'B', 16);

    // Set posisi Y untuk menurunkan tulisan
    $pdf->SetY(14); // Atur nilai ini sesuai kebutuhan
    ...
    // Set posisi Y untuk menurunkan tulisan
    $pdf->SetY(44); // Atur nilai ini sesuai kebutuhan
    ...
    while ($row = mysqli_fetch_array($data_siswa)) {
        ...
        $pdf->Cell(15, $tinggi_sel, $hitung, 1, 0, 'C');
...
if (isset($_POST['create'])) {
    ...
    if($error===true){
        ...
        echo '<script type="text/javascript">window.location.href = "../siswa/?tambah";</script>';
    } 
    else {

        // pengecekan username
    $cek_username = mysqli_query($con, "SELECT * FROM siswa WHERE username = '$username'");
    if (mysqli_num_rows($cek_username) > 0) {
        ...
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $targetFilePath)) {
            ...
            $createsiswa= mysqli_query($con, "INSERT INTO siswa(fullname, address, kelas, phone_number, gender, photo_filename, username, password) VALUES ('$fullname','$address', '$kelas', '$phone_number', '$gender', '$photoFileName','$username','$password')");
            $_SESSION['success'] = "Berhasil menambah data anggota baru";
            echo '<script type="text/javascript">window.location.href = "../siswa";</script>';
        } else {
...
if (isset($_POST['update'])) {
    ...
    if ($error === true) {
        ...
        if ($errphone === true) {
            $_SESSION['error'] .= "<br> - Nomor telepon tidak boleh lebih dari 15 digit.";
        }
        ?>

        <form id="myForm" role="form" method="post" action="../siswa/">
            <input type="hidden" name="id_siswa" value="<?=$id_update?>">
        ...
        // 🔥 Tambahkan pengecekan username tidak boleh sama di sini
        $cek_username = mysqli_query($con, "SELECT * FROM siswa WHERE username = '$username' AND id != '$id_update'");
        if (mysqli_num_rows($cek_username) > 0) {
            $_SESSION['error'] = "Username sudah digunakan oleh anggota lain. Silakan pilih username lain.";
            ?>
            <form id="myForm" role="form" method="post" action="../siswa/">
                <input type="hidden" name="id_siswa" value="<?=$id_update?>">
                <input type="hidden" name="edit" value="edit_value">
        ...
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $targetFilePath)) {
            $getPhotoFilename = mysqli_query($con, "SELECT photo_filename FROM siswa WHERE id = '$id_update'");
            $row = mysqli_fetch_assoc($getPhotoFilename);
            $photoFilename = $row['photo_filename'];
            $photoPath = "../../assets/images/siswa/" . $photoFilename;
            if (file_exists($photoPath) && is_file($photoPath)) {
                unlink($photoPath);
            }

            if ($_POST['password'] == "") {
                $update = mysqli_query($con, "UPDATE siswa SET fullname = '$fullname', address = '$address', kelas='$kelas', phone_number = '$phone_number', gender = '$gender', photo_filename='$photoFileName', username='$username' WHERE id = '$id_update'");
            } else {
                $update = mysqli_query($con, "UPDATE siswa SET fullname = '$fullname', address = '$address', kelas='$kelas', phone_number = '$phone_number', gender = '$gender', photo_filename='$photoFileName', username='$username', password='$password' WHERE id = '$id_update'");
            }
            ...
            if ($update) {
                ...
                $_SESSION['error'] = "Gagal mengubah data anggota";
            }
        } else {
            if ($_POST['password'] == "") {
                $update = mysqli_query($con, "UPDATE siswa SET fullname = '$fullname', address = '$address', kelas='$kelas', phone_number = '$phone_number', gender = '$gender', username='$username' WHERE id = '$id_update'");
            } else {
                $update = mysqli_query($con, "UPDATE siswa SET fullname = '$fullname', address = '$address', kelas='$kelas', phone_number = '$phone_number', gender = '$gender', username='$username', password='$password' WHERE id = '$id_update'");
            }
...
if (isset($_POST['delete'])) {
    ...
    try {
        ...
        $getPhotoFilename = mysqli_query($con, "SELECT photo_filename FROM siswa WHERE id = '$id'");
        $row = mysqli_fetch_assoc($getPhotoFilename);
        $photoFilename = $row['photo_filename'];

        // Attempt to delete the siswarecord
        $delete_siswa= mysqli_query($con, "DELETE FROM siswa WHERE id = '$id'");
    ...
    <div class="col-lg-12 d-flex align-items-stretch">
        <div class="card w-100">
            <div class="card-body p-4">
                    ...
                    <table class="table text-nowrap mb-0 align-middle">
                        ...
                        <tbody>
                            ...
                            if(isset($_POST['search'])){
                                ...
                                $result = mysqli_query($con, "SELECT * FROM siswa WHERE fullname LIKE '%$search_name%'");
                            } else {
                            ...
                            <tr>
                                ...
                                <td class="border-bottom-0">
                                    <h6 class="fw-semibold mb-1"><?= $siswa['fullname'] ?></h6>
                                </td>
                                <td class="border-bottom-0"
                                    style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;">
                                    <p class="mb-0 fw-normal"><?= $siswa['address'] ?></p>
                                </td>

                                <td class="border-bottom-0">
                                    <h6 class="fw-semibold mb-0 fs-4"><?= $siswa['phone_number'] ?></h6>
                                </td>
                                <td class="border-bottom-0">
                                    <div class="d-flex align-items-center gap-2">
                                        <?php 
                                        $gender = strtoupper(substr($siswa['gender'], 0));
                                 ...
                                 <td class="border-bottom-0"
                                    style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;">
                                    <p class="mb-0 fw-normal"><?= $siswa['kelas'] ?></p>
                                </td>
                                 <td class="border-bottom-0">
                                    <h6 class="fw-semibold mb-0 fs-4"><?= $siswa['username'] ?></h6>
                                </td>
                                <td class="border-bottom-0 text-center">
                                    <form role="form" method="post" action="../siswa/">
                ...
                <?php }  
                if(isset($_POST['edit'])){ 
                    $editsiswa= mysqli_query($con, "SELECT * FROM siswa WHERE id=$_POST[id_siswa] ");  
                    $old_data = mysqli_fetch_array($editsiswa)  
                ?>
                    ...
                    <div class="card-body">
                        <form role="form" method="post" action="./index.php" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="image-preview-container mb-3">
                                    ...
                                    <img class="img-thumbnail rounded-circle image-preview" alt="Preview"
                                        src="../../assets/images/siswa/<?= $old_data['photo_filename'] ?>"
                                        style="width: 200px; height: 200px;">
                                    <?php } else { ?>
                                    <img class="img-thumbnail rounded-circle image-preview" alt="Preview"
                            ...
                            <div class="mb-3">
                             ...
                             <div class="mb-3">
                                <label for="kelas" class="form-label">kelas</label>
                                <input type="text" class="form-control" value="<?=$old_data['kelas'] ?>" id="kelas"
                                placeholder="Masukkan kelas" name="kelas" required>                                
                            </div>
                            <div class="mb-3">
                                <label for="phone_number" class="form-label">Nomer Telepon</label>
                            ...
                            <div class="mb-3">
                                <label for="username" class="form-label">Username</label>
                                <input type="text" class="form-control" id="username"
                                placeholder="Masukkan Username" name="username" value="<?= $old_data['username'] ?>" required>                                
                            </div>
                            <div class="mb-3">
                                <label for="password" class="form-label">password</label>
                                <input type="password" class="form-control" id="password"
                ...
                if(isset($_POST['details'])){ 
                    $detailssiswa= mysqli_query($con, "SELECT * FROM siswa WHERE id=$_POST[id_siswa] ");  
                    $details = mysqli_fetch_array($detailssiswa) 
                ?>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="mb-3 d-flex">
                            <div class="mb-3">
                                ...
                                <img class="img-thumbnail rounded-circle" alt="Preview"
                                    src="../../assets/images/siswa/<?= $details['photo_filename'] ?>"
                                    style="width: 200px; height: 200px;">
                                <?php } else { ?>
                                <img class="img-thumbnail rounded-circle" alt="Preview"
                        ...
                        <div class="mb-3 d-flex">
                         ...
                         <div class="mb-3 d-flex">
                            <h4 class="fw-semibold mb-0 fs-6 col-3 "> Username </h4>
                            <h4 class="fw-semibold mb-0 fs-6 col-1 "> : </h4>
                            <h4 class="fw-semibold mb-0 fs-6 col-6"> <?= $details['username'] ?></h4>
                        </div>
                    </div>
                </div>
